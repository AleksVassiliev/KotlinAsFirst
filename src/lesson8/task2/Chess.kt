...
data class Square(val column: Int, val row: Int) {
     ...
     */
    fun notation(): String = if (inside()) String.format("%s%d", columnName[column - 1], row) else ""

    enum class Color {
        WHITE, BLACK
    }

    fun color(): Color = if (row % 2 == column % 2) Color.BLACK else Color.WHITE;
}
...
fun square(notation: String): Square {
    if (notation[0] !in Square.columnName) throw(IllegalArgumentException());
    val c = Square.columnName.indexOf(notation[0]) + 1;
    val r = notation[1].toInt() - 48;
    if (r !in 1..8) throw(IllegalArgumentException());
    return Square(c, r);
}
 ...
 * Если возможно несколько вариантов самой быстрой траектории, вернуть любой из них.
 */
fun diagonal(s: Square): Pair<List<Square>, List<Square>> {
    var diag1 = mutableListOf<Square>();
    var sq = s;
    while (sq.column > 1 && sq.row > 1) sq = Square(sq.column - 1, sq.row - 1);
    while (sq.inside()) {
        diag1.add(sq);
        sq = Square(sq.column + 1, sq.row + 1);
    }
    var diag2 = mutableListOf<Square>();
    sq = s;
    while (sq.column > 1 && sq.row < 8) sq = Square(sq.column - 1, sq.row + 1);
    while (sq.inside()) {
        diag2.add(sq);
        sq = Square(sq.column + 1, sq.row - 1);
    }
    return Pair(diag1, diag2);
}

fun diagonalIntersect(a: List<Square>, b: List<Square>): Square? {
    for (elem in a) {
        if (elem in b) return elem;
    }
    return null;
}
    ...
    if (start.color() == end.color()) {
        ...
        if (start != end) {
            if (abs(start.row - end.row) != abs(start.column - end.column)) {
                val d1 = diagonal(start);
                val d2 = diagonal(end);
                var p = diagonalIntersect(d1.first, d2.second);
                if (p == null) p = diagonalIntersect(d1.second, d2.first);
                result.add(p!!);
            }
            result.add(end);
        }
...
fun kingTrajectory(start: Square, end: Square): List<Square> {
    var result = mutableListOf(start);
    if (start != end) {
        var row = end.row.compareTo(start.row);
        var col = end.column.compareTo(start.column);
        var s = start;
        while (s != end) {
            s = Square((s.column + col), (s.row + row));
            if (s.row == end.row) row = 0;
            if (s.column == end.column) col = 0;
            result.add(s);
        }
    }
    return result;
